...
notation using the Claude API. Falls back to rule-based conversion
if the API is unavailable.

Flow:
    1. Receive English sentence
    2. Send to Claude API with SASL grammar rules (system prompt)
...
class SASLTransformer:
    ...
    Uses the Claude API as the primary translation engine, with a
    ...
    async def _translate_with_llm(
        ...
        """
        Use the Claude API to translate English → SASL gloss.
        ...
        message = await self._client.messages.create(
            ...
            max_tokens=settings.sasl_max_tokens,
            temperature=settings.sasl_temperature,
